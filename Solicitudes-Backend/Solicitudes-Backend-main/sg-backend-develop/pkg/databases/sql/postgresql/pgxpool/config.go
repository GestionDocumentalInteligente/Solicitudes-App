...
func newConfig(user, password, host, port, migrationsDir, dbName string) defs.Config {
	return &config{
		Host:          host,
		User:          user,
		Password:      password,
...
func (c *config) DNS() string {
	return fmt.Sprintf("postgres://%s:%s@%s:%s/%s?sslmode=%s", c.User, c.Password, c.Host, c.Port, c.DbName, os.Getenv("SSL_MODE"))
}
